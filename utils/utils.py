...
def chat(prompt, model="gpt-4.1", max_tokens=28000):
    ...
    if model in ["gpt-4o", "gpt-4.1"]:
        ...
        print("Using OpenAI load api key in dotenv")
        # client = OpenAI()
    elif model in ["claude-3-opus", "claude-3-7-sonnet", "claude-3-5-haiku"]:
        model_provider = "anthropic"
        print("Using anthropic load api key in dotvenv")
...
def get_char_to_token_map(text, tokenizer):
    ...
    token_offsets = tokenizer.encode_plus(text, return_offsets_mapping=True)[
        "offset_mapping"
    ]
...
def load_model_and_vectors(
    ...
    def get_model_path(name):
        """Convert HuggingFace repo_id to HPC shared storage path, download if needed"""
        if name is None:
            return None
        if name.startswith("/"):
            return name
        if "/" in name:
            shared_path = f"/Brain/public/models/{name}"

            if os.path.exists(shared_path):
                print(f"✓ Loading {name} from shared storage: {shared_path}")
                return shared_path
            else:
                print(f"Model not found at {shared_path}")
                print("Downloading to shared storage for everyone to use...")
                ...
                try:
                    ...
                    snapshot_download(
                        repo_id=name,
                        local_dir=shared_path,
                        local_dir_use_symlinks=False,
                    )

                    import subprocess

                    subprocess.run(["chmod", "-R", "777", shared_path], check=False)

                    print(f"✓ Downloaded and shared at: {shared_path}")
                    return shared_path

                except Exception as e:
                    print(f"❌ Failed to download to shared storage: {e}")
    ...
    base_model_path = get_model_path(base_model_name)
    model = LanguageModel(
        model_path,
        dispatch=True,
            ...
            for label in [
                ...
                if label != "overall":
                    feature_vectors[label] = (
                        mean_vectors_dict[label]["mean"]
                        - mean_vectors_dict["overall"]["mean"]
                    )

                if normalize_features:
                    for label in feature_vectors:
                        for layer in range(model.config.num_hidden_layers):
                            feature_vectors[label][layer] = feature_vectors[label][
                                layer
                            ] * (
                                feature_vectors["overall"][layer].norm()
...
def custom_generate_steering(
    ...
    with model.generate(
        ...
        if feature_vectors is not None:
            ...
            coefficient = (
                steering_config[label]["pos_coefficient"]
                if steer_positive
                else steering_config[label]["neg_coefficient"]
            )

            if steer_positive:
                feature_vector = (
                    feature_vectors[label][vector_layer].to("cuda").to(torch.bfloat16)
                )
                for layer_idx in pos_layers:
                    model.model.layers[layer_idx].output[0][:, :] += (
                        coefficient * feature_vector.unsqueeze(0).unsqueeze(0)
                    )
            else:
                feature_vector = (
                    feature_vectors[label][vector_layer].to("cuda").to(torch.bfloat16)
...
def get_batched_message_ids(tokenizer, messages_list):
    ...
    max_token_length = max(
        [len(tokenizer.encode(msg, return_tensors="pt")[0]) for msg in messages_list]
    )
    input_ids = torch.cat(
        [
            tokenizer.encode(
...
def process_saved_responses_batch(responses_list, tokenizer, model):
    ...
    for batch_idx in range(len(responses_list)):
        ...
        attention_mask = (
            tokenized_responses[batch_idx] != tokenizer.pad_token_id
        ).long()
        ...
        example_outputs = torch.stack(
            [layer_output[batch_idx][padding_length:] for layer_output in layer_outputs]
        )
